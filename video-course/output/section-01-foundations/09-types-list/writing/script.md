...
The pitfall is copy-paste repetition. Five hand-typed subnet blocks are five chances to mistype a prefix and five edits for every change. The list plus for expression pattern collapses that into data. One note for the next lab: lists are perfect when entries are interchangeable. When each entry needs its own identity, like a name you reference later, maps are the better fit, and that's exactly where we're headed.
...
Quick recap. A list is ordered values, addressed by index. A for expression transformed that list into objects with computed names. A dynamic block emitted one subnet per entry, and the output projected the names back out. The habit to keep: repetition should live in data, not in repeated code.
