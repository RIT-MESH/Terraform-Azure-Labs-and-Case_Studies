...
Here are the pitfalls in this lab. One: sensitive masks logs only — the value still sits in state, in plain text. Two: don't output secrets that no downstream consumer needs. Three: terraform output, followed by a name, prints the real value on demand — by design. Four: state-file protection is a separate concern — not this flag's job.
