...
Here are the pitfalls in this lab. One: t f vars must match the map of object type exactly — misspelled or missing fields are hard errors. Two: the rejection happens before any plan — fail fast, by design. Three: keeping an unused field in the shape is normal while a module grows. Four: adding a field later breaks every caller — design the shape up front.
