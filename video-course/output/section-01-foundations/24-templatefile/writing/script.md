...
Here are the pitfalls in this lab. One: percent brace and dollar brace inside a comment are still parsed as directives. Two: a stray dollar brace in the template breaks rendering at plan time. Three: custom data runs cloud-init on first boot only — changes need care. Four: use the percent brace tilde form to trim whitespace, so a loop doesn't leave blank lines.
