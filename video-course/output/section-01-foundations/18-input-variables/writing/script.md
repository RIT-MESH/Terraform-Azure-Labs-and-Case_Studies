...
Here are the pitfalls in this lab. One: defaults are silent — a misspelled region or tier passes straight through. Two: validation runs before plan — it's your input gate, so use it. Three: the error message should say what to fix, not just that something failed. Four: validation is for input rules — not for resource behavior.
